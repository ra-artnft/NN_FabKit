...
**NB (v0.12.0+):** в FabKit CAD актуальная реализация считает `theta_deg`
из joint-to-far vectors (не из axis vectors с `.abs`), и `mitre = (180 − θ)/2`.
Это даёт корректный ответ для всего диапазона:

- θ=90° → mitre 45°
- θ=60° → mitre 60° (Y/K brace)
- θ=120° → mitre 30° (тупой L)

До v0.12.0 формула была `acos(axis_a.dot(axis_b).abs) / 2` — давала верный
mitre **только** в случае, когда обе трубы имеют local +Z, направленный
«away from joint» (классическая L-конфигурация с обеими `end_axis=+1`).
В Y-конфигурациях (`end_axis=−1` для одной или обеих) и в тупых L
формула давала неверный mitre. Баг fixed v0.12.0 (см. CHANGELOG).
...
|---|---|---|---|---|---|---|---|
| **Rect (W×H)** | ✅ v0.11.5 (mitre 45/45 + trim) | ✅ v0.12.0 (joint-to-far formula, work для всего θ) | 🔜 этап 2 (mitre=0) | 🔜 этап 3 (envelope cope) | 🟡 не приоритет | 🔜 этап 4 | ⏸ далеко |
| **Square (W=W)** | ✅ как rect (W=H) | ✅ v0.12.0 | 🔜 этап 2 | 🔜 этап 3 | 🟡 не приоритет | 🔜 этап 4 | ⏸ далеко |
...
- Joint angle θ = full angle (0..180°), через **joint-to-far vectors**
  (v0.12.0+; ранее `axis_a.dot(axis_b).abs` сворачивал в acute и ломался
  на Y-конфигурациях).
- Mitre angle α = (180° − θ)/2. Корректно для всего диапазона: 60° brace
  → α=60°; 90° L → α=45°; 120° тупой L → α=30°.
- Skew axes detection (v0.12.0+): если оси не пересекаются (skew > 1mm),
  tool явно abort'ится с messagebox-ом «подвинь трубу так, чтобы оси
  встретились» вместо silent gap.
- Параллельные/коллинеарные трубы (`θ < 1°`) детектируются и отклоняются
  с понятным сообщением.
- Tilt direction = вектор от joint к far_end other tube, projected на
  cross-section plane self tube. Работает для всех 4 end_axis комбинаций
  (v0.11.10+).
- Trim mode (default ON): каждая труба shortened/extended до axis
  intersection (`Geom.closest_points`) перед mitre — `length_mm`
  обновляется.
...
- Auto `make_unique` если definition shared (v0.11.12+).
- Snap markers в углах bbox (8 углов × 3 edges-кресты, magenta, dedicated
  Tag `FabKit::SnapMarkers`) — видны всегда, не зависят от
  `View → Construction Geometry` toggle (v0.12.0+).

**Ограничения:**
- Только rect_tube (square = частный case rect).
- Только L-mitre 2-tube selection.
- Symmetric mitre (одинаковый α на обеих) — для разных сечений см.
  [3.5](#35-asymmetric-mitre-разные-сечения), defer to cope joint mode
  (v0.13+).
...
| 1.1 | Fix θ > 90° (тупой L-corner) | ✅ done | v0.12.0 |
| 1.2 | Skew axes detection + abort | ✅ done | v0.12.0 |
| 1.3 | Snap markers always-visible (edges + dedicated Tag) | ✅ done | v0.12.0 |
| 2 | T-joint butt (perpendicular cut на brace) | 🔜 next | v0.13.x |
| 3 | T-joint notch (envelope cope rect-on-rect) | 🔜 | v0.14.x |
| 4 | X-cross 4-tube symmetric | 🔜 | v0.15.x |
| 5 | Asymmetric L-mitre (разные сечения) | 🟡 | tbd |
| 6 | Round-tube L-mitre (plane cut → эллипс) | 🟡 | v0.16.x |
| 7 | Round-on-round fishmouth cope | 🟡 | v0.17.x |
| 8 | Y / K фермы | ⏸ | tbd |
...
1. ~~**Тупой L-corner (θ > 90°)**~~ — **resolved v0.12.0**. Формула
   переписана через joint-to-far vectors: `θ = acos((far_a − end_a) ·
   (far_b − end_b))`, mitre = (180 − θ)/2. Корректно для всего
   диапазона включая Y-конфигурации.

2. ~~**Skew axes**~~ — **resolved v0.12.0** через выбор (б): tool
   проверяет skew distance после `Geom.closest_points` и при
   превышении 1mm (`SKEW_TOLERANCE_MM`) abort'ится с messagebox-ом
   объясняющим почему стык невозможен и что подвинуть. Лучше явный
   отказ, чем silent gap. Альтернатива (а) с midpoint trim даёт
   skew, видимый на side faces — это portable artifact, который
   у фабрикатора вызовет вопросы; не делаем.
