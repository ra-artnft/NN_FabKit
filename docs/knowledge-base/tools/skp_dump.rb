...
# как входной контекст. Предназначен для копипаста в Ruby Console.
#
# Применение:
#   1. Открыть модель в SketchUp.
#   2. Window → Ruby Console.
#   3. Скопировать всё содержимое этого файла, вставить в консоль, Enter.
#   4. Последняя строка напечатает путь к созданному JSON.
#
# Поведение:
#   - Без параметров: сохраняет рядом с .skp-файлом с именем <basename>.dump.json.
#   - Если модель не сохранена (нет path) — сохраняет в рабочий стол пользователя
#     с timestamp'ом в имени.
#   - Явно указать путь: NnFabKit::SkpDump.run(path: "C:/tmp/my.dump.json")
#
...
# ============================================================================
# Автозапуск при вставке в Ruby Console.
# Если скрипт подключается через Sketchup.require как модуль, автозапуска
# не будет — вызывайте NN::FabKit::SkpDump.run(path: "...") вручную.
# ============================================================================

NN::FabKit::SkpDump.run if defined?(Sketchup) && Sketchup.active_model
