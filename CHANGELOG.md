...
### Added
- Плагин v0.4.0: команда `Extensions → NN FabKit → MetalFab → Демо: рамка перегородки 420×2210` — воспроизводит внешнюю рамку перегородки заказчика-0 из модели `02-okazov-peregorodka` (4 трубы 40×20×2 в плоскости YZ — 2 вертикали по 2210мм + 2 горизонтали, вписанные между вертикалями длиной 340мм). Это Sprint A: без запилов 45° на торцах. Sprint B заменит 340мм-горизонтали на 420мм с миттер-резами через Solid Tools `subtract` (доступно в SU Pro у заказчика).
- Анализ дампа `02-okazov-peregorodka`: подтверждена двухрамочная компоновка (внешняя 40×20 рамка 420×2210 + внутренняя 15×15 рамка 380×2170 + 2 листа стекла 4мм между ними + крепёжные пластинки + контекстная рама проёма 100×140). Признак запилов 45° — горизонтали имеют bounds 40×420×20 (полная ширина = 420, не вписаны), а не 40×340×20.
