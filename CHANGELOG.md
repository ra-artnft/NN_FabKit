...
- **Плагин 0.12.1 → 0.12.2** — feedback пользователя по PDF-чертежу:
  «нужны размеры (ширина/высота) и спецификация по деталям, у нас 4 одинаковых трубы вот такой длины».

#### Cut-list: колонка «Длина, мм» + группировка по (typesize × length)
- **Было**: группировка только по `typesize`, в строке только Σ Длина (агрегированная). Не видно длину одной детали — приходилось делить Σ/Кол-во в уме.
- **Стало**: группировка по паре `(typesize × length_per_piece)`, новая колонка «Длина, мм» = длина одной штуки. Каждая уникальная пара = одна строка. Например: рама 1000×600 даст 2 строки (1000 мм × 2 шт + 600 мм × 2 шт), а раньше — 1 строку «4 шт / Σ 3200».
- Группы сортируются по typesize, потом по убыванию длины — стабильный порядок.

#### Размеры по осям в подписях viewport
- Новый helper `axis_bbox_mm(model)` — bbox по **осям** rect_tube инстансов (т.е. по axis intersections в углах рамы), а не по outer geometry. Для рамы 1350 из трубы 40 это даёт **1350×1350**, а не 1390×1390 (которое включало бы толщину сечения).
- Подписи viewport-ов теперь дополнены автоматически:
  - «Сверху (1350 × 1350 мм)» — width × depth
  - «Спереди (1350 мм)» — width (depth = 0 для плоской рамы)
  - «Сбоку (справа) (1350 мм)» — depth (height = 0)
- Изометрия — без размеров (избыточно, размеры уже видны на ortho viewport-ах).

### Modified
- `plugin-sketchup/src/nn_fabkit/metalfab/layout/template_cut_list.rb` — `CUT_LIST.cols` (новая «Длина, мм»), `group_rect_tubes` (group by typesize × length), `draw_cut_list` (новая колонка), `collect_cut_list` (totals), новые helpers `axis_bbox_mm`, `augment_label`.
